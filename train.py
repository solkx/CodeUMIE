...
from model.common.registry import registry
import argparse
...
import yaml
import torch
from model.processors.video_processor import ToTHWC,ToUint8,load_video
import bitsandbytes as bnb
from collections import defaultdict
from peft import LoraConfig, get_peft_model, prepare_model_for_kbit_training
from transformers import AutoTokenizer
from model.models.modeling_llm import llmForCausalLM, MutliModel
...
def insert_adapter(args, model, Myconfig):
    ...
    llm_model = get_peft_model(model.model, config)
    model = get_peft_model(model, config)
    model.base_model.model.model = llm_model.base_model.model
...
from os.path import join
from loguru import logger
def merge_lora_to_base_model(model_name_or_path, adapter_name_or_path):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        trust_remote_code=True,
        # llm不支持fast
        use_fast=False if config.model_type == 'llm' else True
    )
...
def pred(model, training_args, test_dataset, tokenizer, args):
    ...
    if os.path.exists(f"./{best_model_path}/{best_filename}/adapter_model.safetensors"):
        fintuneModel_path = f"./{best_model_path}/{best_filename}/adapter_model.safetensors"
    else:
        fintuneModel_path = f"./{best_model_path}/adapter_model.safetensors"
    # fintuneModel = torch.load(fintuneModel_path)
    fintuneModel = load_file(fintuneModel_path)
    ...
    llm_proj_dict = torch.load(f"./{best_model_path}/{best_filename}/llm_proj.bin")
    model_dict.update({f"base_model.model.video_Qformer.{k}":v for k,v in video_Qformer_dict.items()})
    model_dict.update({f"base_model.model.llm_proj.{k}":v for k,v in llm_proj_dict.items()})

    model.load_state_dict(model_dict)
    model = model.eval()
    llm_model = model.base_model.model.model
    load_in_4bit = False
    # 生成超参配置
    max_new_tokens = 4096
    top_p_list = [0.9]
                ...
                for j in range(0, 1):
                    ...
                    with open(f"./result_{outputRoot}/{config.dataset}-{config.template}-{config.model_type}-{training_args.seed}-{training_args.learning_rate}.json", "a+", encoding="utf-8") as f:
                        for i, data_batch in enumerate(tqdm(test_dataset)):
                            ...
                            embed_tokens = llm_model.model.embed_tokens
                            inputs_embeds = None
                            ...
                            llm_proj = model.base_model.model.llm_proj
                            ...
                            inputs_llm_video = video_query_output.last_hidden_state

                            inputs_llm_video = llm_proj(inputs_llm_video)
    
                            src_embeds = embed_tokens(input_ids)
                            result_embeds = embed_tokens(tgt_list)   
                            inputs_embeds = torch.cat([src_embeds.unsqueeze(0), inputs_llm_video, result_embeds.unsqueeze(0)], dim=1)
                            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_args_file", type=str, default='./train.yaml')
    parser.add_argument("--cfg-path", default='./test.yaml')
    parser.add_argument("--model_type", type=str, default="qwen", choices=["qwen"])
