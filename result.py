...
for lang in ["zh", "en"]:
    for name in ["train", "dev"]:
        with open(f'./data/m3d/{lang}/{name}_{lang}.json', 'r', encoding='utf-8') as f:
...
def main(root, mode, lang, lr):
    ...
    for res_name in tqdm(os.listdir(f"./{root}")):
        if f"-{mode}-" not in res_name:
            continue
        c_lang = res_name.split("-")[0].split("_")[1]
        if c_lang != lang:
            continue   
...
if __name__ == "__main__":
    ...
    mode_list = [["result_output_code_m3d_en_ours_code_0.0002_code","llama", "en", 0.0002], ["result_output_code_m3d_zh_ours_code_0.0008_code","baichuan", "zh", 0.0008]]
    tabel_list = []
