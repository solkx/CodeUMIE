...
from model.models.blip2 import Blip2Base, disabled_train
from functools import partial
import einops
from model.models.Qformer import BertConfig, BertLMHeadModel
from model.models.ImageBind.models.imagebind_model import ModalityType
from model.models.ImageBind.models import imagebind_model
...
class model(Blip2Base):
    ...
    PRETRAINED_MODEL_CONFIG_DICT = {
        "pretrain_vicuna": "configs/models/model.yaml",
        "pretrain_v2": "configs/models/model.yaml",
    }
    ...
    def forward(self,
        ...
        for i in range(len(input_ids)):
            ...
            inputs_llm_video = self.llm_proj(inputs_llm_video)
            extent_attention_video = torch.ones([inputs_llm_video.shape[0], inputs_llm_video.shape[1]]).cuda()
            ...
            labels = torch.cat([input_ids[i].unsqueeze(0), torch.tensor([-100]*inputs_llm_video.shape[1]).cuda().unsqueeze(0).repeat(1, 1), labels[i].unsqueeze(0)], dim=1)
    ...
    def from_config(cls, cfg):
        ...
        llm_proj_model = cfg.get("llm_proj_model", '')
        ...
        if ckpt_path:
            ...
            del ckpt['model']["llm_proj.weight"]
            del ckpt['model']["llm_proj.bias"]
            msg = model.load_state_dict(ckpt['model'], strict=False)
        ckpt_path_2 = cfg.get("ckpt_2", "")  
        if ckpt_path_2:
            print("Load second Checkpoint: {}".format(ckpt_path_2))
            ckpt = torch.load(ckpt_path_2, map_location="cpu")
            del ckpt['model']["audio_llm_proj.weight"]
            del ckpt['model']["audio_llm_proj.bias"]
            msg = model.load_state_dict(ckpt['model'], strict=False)
        return model
